...
def parse_requirement(req):
    ...
    if remaining and remaining[0] != '#':
        raise SyntaxError('unexpected trailing fixtures: %s' % remaining)
...
def _get_external_data(url):
    ...
    try:
        ...
        if not ct.startswith('application/json'):
            ...
            # fixtures = reader.read().decode('utf-8')
            # result = json.loads(fixtures)
            result = json.load(reader)
    except Exception as e:
        logger.exception('Failed to get external fixtures for %s: %s', url, e)
    return result
